...
public class OperacionesDatos111111 {
    
    private float [] datos;
    private float prom;
       public static void main(String[] x){
        new OperacionesDatos111111();
    }
    public OperacionesDatos111111(){
        this.datos = null;
        this.prom=0;
        this.menu();
        ...
        private void menu(){
            this.configurar();
            float datosorg [] = this.capturaDatos();
        int opc;
        do{
            ...
            case 1:
                ...
                break;
            case 2:
                this.muestraOriginales(datosorg);
                break;
            case 3:
    ...
    private float [] capturaDatos(){ // se le pone los parametros que tiene que recibir de otro metodo
        float [] datosorg;
        datosorg =  new float [this.datos.length];
        for(int i=0;i<this.datos.length;i++){// se coloca el this en la variable para mostrar que es una variable de clase
            ...
            datosorg [i] = this.datos [i];
        }
        return datosorg;
    }
    private void muestraOriginales(float [] datosorg){
        for(int i=0;i<this.datos.length;i++){
            System.out.print(datosorg[i]+" ");            
        }
        System.out.println();
